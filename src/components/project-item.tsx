  ...
  return (
    <div>
      ...
      {data.results.map((project: any, index: number) => {
        ...
        // 홀수와 짝수를 구분하여 정렬 적용
        const alignmentClass = index % 2 === 0 ? 'flex-row' : 'flex-row-reverse';
          ...
          >
            ...
            <div className="flex flex-col justify-center w-1/2 px-4">
              ...
              {project.properties.날짜.date && (
                <p className="mb-3">
                  {project.properties.날짜.date.start} ~ {project.properties.날짜.date.end}
                </p>
              )}

              {/* 설명 */}
              <p className="mb-3">{project.properties.설명.rich_text.map((text: any) => text.plain_text).join('')}</p>
              ...
              {project.properties.Github.url && (
                <p>
                  ...
                  <a href={project.properties.Github.url} target="_blank" rel="noopener noreferrer">
                    {project.properties.Github.url}
                  </a>
                </p>
              )}
              ...
              {project.properties.유투브.url && (
                <p>
                  ...
                  <a href={project.properties.유투브.url} target="_blank" rel="noopener noreferrer">
                    {project.properties.유투브.url}
                  </a>
                </p>
              )}
